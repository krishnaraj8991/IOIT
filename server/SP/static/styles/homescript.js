...
$(document).ready(function () {
    ...
    socket.on('redirect',function(msg){
        console.log('redirecting to' + msg.data);
        // $('#' + msg.who).val(msg.data);
        window.location.replace("http://"+msg.data);
        //  msg.data;
        // document.getElementById("slider1").val=0;
    });
});
...
function nextbt() {
    ...
    if (index + 1 <= limit) {
        ...
        // socket.emit('next', "hi");
        // var cs = document.getElementById("current_song")
        // cs.innerHTML = que[index++];
    }
}

function add_v(data, index) {
    // console.log('called')
    var table = document.getElementById("table");
    var row = table.insertRow(-1);
    var cell1 = row.insertCell(0);
    var cell2 = row.insertCell(1);
    cell2.innerHTML = "<a href=\"javascript:list_clicked('" + data + "')\" id=\"foo\">" + data + "</a>";
    // data = data.slice(0, -4);
    // name=url_for('static', filename= data +".jpg");
    cell1.innerHTML = "<img class=\"img_v\" id='" + data + "'/>";
    document.getElementById(data).src = "static\\images\\playerIcon.jpg";
...
function add_h(data) {
    ...
    // cell1.innerHTML = "<img class=\"img_h\" id='" + data + "';style=\"width:200px;height:200px;\"/>";
    cell1.innerHTML ="<a href=\"javascript:list_clicked('"+olddata+"')\">" + 
                        "<img class=\"img_h\" id='" + data + "';style=\"width:200px;height:200px;\"/>" +
                         "</a>";
    
    document.getElementById(data).src="static\\images\\playerIcon.jpg";
    
    //change this according to css temp fixed input
...
function list_clicked(data) {
    console.log(data);
    window.location.replace("http://"+data+":5000");
