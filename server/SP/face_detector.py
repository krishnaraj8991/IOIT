...
# tunnel_ptr
def send_name(name,tunnel):
    print("sending:-"+name)
    # global tunnel_ptr
    tunnel.put(name)

def face_detect(video,pretraker,mode,tunnel):
...
def detector(tunnel,ip):
    ...
    # -----------------------------------------------
    trackers=face_detect(video,None,1,tunnel)
    # -----------------------------------------------
    ...
    while True:
        ...
        if flag:
            ...
            """
            if cnt==100:
                trackers=face_detect(video,trackers,2,tunnel)
            else:
                trackers=face_detect(video,trackers,1,tunnel)
            cnt=0
            # print(len(trackers))
